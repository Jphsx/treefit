...
void writeClass(std::vector<leafinfo*> LV){
	...
	std::string outclassname = path + outfile + ".cpp";
	ofstream outputFile(outclassname);
	...
	outputFile << "class "+outfile+"{ "<< std::endl;
	...
	outputFile <<"		"+outfile+"();"<<std::endl;	
	outputFile << "};"<<std::endl;
	outputFile << outfile+"::"+outfile+"(){"<<std::endl;
	for(int i=0; i< LV.size(); i++){
			if(!strcmp(LV.at(i)->type,"Double_t")){
...
void writeLoop(std::vector<leafinfo*> LV){
	std::string loopname = path+ outfile + "_Loop.cpp";
	ofstream outputFile(loopname);
	outputFile <<"#include \""+outfile+".cpp\""<<std::endl;
	...
	outputFile << std::endl;
	outputFile <<"void "+loopname+"(){"<<std::endl;
	outputFile <<"	TTree* tree=0;"<<std::endl;
	outputFile <<"	TFile *f = TFile::Open(\""+path+infile+"\");"<<std::endl;
	outputFile <<" 	TFile *f2 = new TFile(\""+intree+".root\",\"RECREATE\");"<<std::endl;
	outputFile <<"	f->GetObject(\""+intree+"\",tree);"<<std::endl;
	outputFile <<"  "+outfile+"* oc = new "+outfile+"();"<<std::endl;;
	outputFile <<std::endl;
...
void generateTreeClass(std::string inputfilename, 
