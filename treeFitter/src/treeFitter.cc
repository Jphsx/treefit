...
void treeFitter::FindMassConstraintCandidates(LCCollectionVec * recparcol) {
	...
	for(unsigned int i=0; i<_pfovec.size(); i++){
		Particle* pc = new Particle(_pfovec.at(i), Matching::MatchParticleToTrack(_pfovec.at(i), _trackvec,B),B, _trackFitObject);
				...
				if(VertexObjects.at(i) != NULL){
					...
					for(int k=0; k<fit.at(i).size(); k++){
						if( newparts.at( fit.at(i).at(k) )->isTrack ){
							...
							const float* oldref = TFit->recoparts.at(fit.at(i).at(k))->track->getReferencePoint();							
							std::cout<<"diff "<< fabs(newref.at(0) -(double) oldref[0])<<std::endl;	
							std::cout<<"new - old "<< newref.at(0) <<" "<<oldref[0]<<std::endl;	
